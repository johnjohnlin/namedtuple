...
DEFINE_NAMEDTUPLE(S35)
	NT_MEMBER(NT_TYPE(string),miheffhflnkegngklhdjjlejaealn)
	NT_MEMBER(NT_TYPE(string),enndkbjkfdnfaddidl)
	NT_MEMBER(NT_TYPE(unsigned),eghhdmkalfhihfk)
	NT_MEMBER(NT_TYPE(unsigned),bdiljbcbmhhjmeigdc)
	NT_MEMBER(NT_TYPE(unsigned),ciimlhbijfambki)
	NT_MEMBER(NT_TYPE(long),bbkefnhbggkdminmea)
	NT_MEMBER(NT_TYPE(long),ibgceljinaingekh)
	NT_MEMBER(NT_TYPE(int*),ellilnlbhcfljm)
	NT_MEMBER(NT_TYPE(string),bhffclekdm)
	NT_MEMBER(NT_TYPE(long),hefddnhhamlidhcnbmi)
	NT_MEMBER(NT_TYPE(S30),jaibnijcblakmiifmfklehmlgjbkf)
	NT_MEMBER(NT_TYPE(S27),mbnkdedbbhlbif)
	NT_MEMBER(NT_TYPE(S30),nikgkggnkincngcfgidjlb)
	NT_MEMBER(NT_TYPE(S6),gkkhkbclhkncjedkcdljkggdfkede)
	NT_MEMBER(NT_TYPE(S13),hblkmjkadndnmd)
END_DEFINE_NAMEDTUPLE(S35)
DEFINE_NAMEDTUPLE(S36)
	NT_MEMBER(NT_TYPE(float),glafiidiakjmnifccekcggfgd)
	NT_MEMBER(NT_TYPE(unsigned),iiadddkcfcakelejgffcfeljmej)
	NT_MEMBER(NT_TYPE(float),bfbadenbngbhemkcgeb)
	NT_MEMBER(NT_TYPE(int*),dlgfflacfmnhnkfgnifeg)
	NT_MEMBER(NT_TYPE(float),ggceflafankcnkeiialgbkkh)
	NT_MEMBER(NT_TYPE(string),kbhihnhgkfccnmhfflc)
	NT_MEMBER(NT_TYPE(unsigned),bhlhaciadmggijbceh)
	NT_MEMBER(NT_TYPE(int*),fhdgakijabamihdaebljf)
	NT_MEMBER(NT_TYPE(int*),nbgkcinllfagcdahdhbidme)
	NT_MEMBER(NT_TYPE(int*),jhglidncgn)
	NT_MEMBER(NT_TYPE(S6),bdgeaikbabc)
	NT_MEMBER(NT_TYPE(S25),mgngfbchfakibikghfaaj)
	NT_MEMBER(NT_TYPE(S2),jgmneideeliemfgelgnnk)
	NT_MEMBER(NT_TYPE(S11),maaeghfiabhhebaiikf)
	NT_MEMBER(NT_TYPE(S11),nahlgejmldnh)
END_DEFINE_NAMEDTUPLE(S36)
DEFINE_NAMEDTUPLE(S37)
	NT_MEMBER(NT_TYPE(string),dcdblhidea)
	NT_MEMBER(NT_TYPE(unsigned),fjcamnfjkjndfemjbfkijhdfnij)
	NT_MEMBER(NT_TYPE(long),dgjfkiljfmkddklfemfnkhbkg)
	NT_MEMBER(NT_TYPE(int*),gjdhendlencfniacklccgn)
	NT_MEMBER(NT_TYPE(float),jkhliiekdlmbgglknhfjkdfjfi)
	NT_MEMBER(NT_TYPE(long),hjgbhhkkhnhjbgkeleknjff)
	NT_MEMBER(NT_TYPE(long),ejjcifjblghgnbhjn)
	NT_MEMBER(NT_TYPE(float),kbbkcambdidnhmafcmennhmgecbe)
	NT_MEMBER(NT_TYPE(float),hnggkmjnnkblhdlidelikcldmj)
	NT_MEMBER(NT_TYPE(string),fhdhcdmhlajneg)
	NT_MEMBER(NT_TYPE(S11),gafbafeicmald)
	NT_MEMBER(NT_TYPE(S31),ejakkjcfnggbiaaddaclalfma)
	NT_MEMBER(NT_TYPE(S11),aeljlmndanjcgiiljgfembgbcb)
	NT_MEMBER(NT_TYPE(S27),hiffedemedkdilmdlbbkcgbcbbjid)
	NT_MEMBER(NT_TYPE(S34),hkffhdhndenbciemednehliinlchi)
END_DEFINE_NAMEDTUPLE(S37)
DEFINE_NAMEDTUPLE(S38)
	NT_MEMBER(NT_TYPE(unsigned),nkhlgndhilcm)
	NT_MEMBER(NT_TYPE(long),ecdndggknh)
	NT_MEMBER(NT_TYPE(int*),abnfbfbknkjg)
	NT_MEMBER(NT_TYPE(long),ahnigbkcgm)
	NT_MEMBER(NT_TYPE(unsigned),gibebekkmnkfjblnfjim)
	NT_MEMBER(NT_TYPE(unsigned),fenbmnddla)
	NT_MEMBER(NT_TYPE(string),keahhambcnbehmhajff)
	NT_MEMBER(NT_TYPE(unsigned),ldhmcmaelchlhgndn)
	NT_MEMBER(NT_TYPE(float),eggjjiekfkjchbnckimkdhfci)
	NT_MEMBER(NT_TYPE(unsigned),bckgebllfhfkbdifbmdmnbh)
	NT_MEMBER(NT_TYPE(S10),fieigfnjhkdhbilbdagkcn)
	NT_MEMBER(NT_TYPE(S18),khamifejmcfamhljdemfenjain)
	NT_MEMBER(NT_TYPE(S5),jhcedldkihbgndkkhcjimiciinm)
	NT_MEMBER(NT_TYPE(S34),jlnjmenkhelndjegnjlhnighdcbhbh)
	NT_MEMBER(NT_TYPE(S23),makfimmmbghdecbdgmmabbhkejc)
END_DEFINE_NAMEDTUPLE(S38)
DEFINE_NAMEDTUPLE(S39)
	NT_MEMBER(NT_TYPE(float),gbfgenbbhlcclmacdeckkhjndf)
	NT_MEMBER(NT_TYPE(string),fimcfggkfdhakhbbdgjllggbfan)
	NT_MEMBER(NT_TYPE(float),nfflhmmhbhnbabjlbi)
	NT_MEMBER(NT_TYPE(float),nkggbefkgm)
	NT_MEMBER(NT_TYPE(string),kkjdclkeffl)
	NT_MEMBER(NT_TYPE(int*),blhncmagcjemagkadgbglgff)
	NT_MEMBER(NT_TYPE(string),gmacjlhgaehclnkckcmfjbfdid)
	NT_MEMBER(NT_TYPE(float),lmkalafbkhbdfakhhekidadbg)
	NT_MEMBER(NT_TYPE(string),edeheggnehehjfnngbbncdjfdmggj)
	NT_MEMBER(NT_TYPE(string),gkjbgehgealkmedkchbhaki)
	NT_MEMBER(NT_TYPE(S37),lnfibnkhfjdmblaea)
	NT_MEMBER(NT_TYPE(S9),icjhkjalmnimalcjdina)
	NT_MEMBER(NT_TYPE(S18),lgjbkmajnnihdmchcmc)
	NT_MEMBER(NT_TYPE(S4),fbigabggegggjckjkknn)
	NT_MEMBER(NT_TYPE(S21),haacangamkmejcemdininaekicnijm)
END_DEFINE_NAMEDTUPLE(S39)
